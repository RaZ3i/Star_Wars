const starship = document.querySelectorAll('.starship_card');
...
for (const element of starship) {
    ...
    try {
        let response = await fetch(`http://127.0.0.1:8001/home_page/starships/starship_info/${e.target.id}`, {
            method: "GET",
            }
        );
        if (response) {
